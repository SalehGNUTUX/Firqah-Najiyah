...
## سجلّ الحالة (يُحدَّث أولاً بأول)

- **2026-07-10 — Phase 0 منجزة بالكامل ومرفوعة (commit c8fe026):** سقالة Gradle/Compose تُبنى فعلياً، 8 شاشات تعمل، `ContentRepository` (أوفلاين + تحديث اختياري) مكتمل، محتوى الصفحات الثابتة الخمس + 3 فرق نموذجية (ashairah, shia, druze).
- **2026-07-10 — الموسوعة اكتملت 18/18 (commit 75a15d8):** بقية الفرق الـ15 نُقلت إلى JSON عبر 15 وكيلاً متوازياً، كل ملف تحقَّق من صحة JSON وخلوّه من HTML/رموز عارضة. `sects/index.json`: hasDetail=true للجميع. `manifest.json`: نسخة 2 (25 ملفاً). أُعيد البناء بنجاح ورُفع.
- **2026-07-10 — أول تجربة فعلية ناجحة على جهاز حقيقي (commit a228805):** ثبَّت المستخدم `Firqah-Najiyah-debug.apk` يدوياً على هاتفه الحقيقي (لا يوجد محاكي جاهز — صورة النظام غير مثبَّتة) وأكّد أنه يعمل. بعدها: (1) روابط المصادر أصبحت قابلة للنقر (`SourceLinkRow` + حقل `links` جديد في `ContentSection`)، (2) اسم حزمة APK الناتجة أصبح `Firqah-Najiyah-debug.apk` (`base.archivesName`) بدل `app-debug.apk`. حزمة الاختبار تُنسخ دائماً إلى `~/Downloads/Firqah-Najiyah-debug.apk`.
- **2026-07-10 — الإصدار 1.0.0 منشور رسمياً (commit e9f6a44، وسم v1.0.0):** `versionCode=2`، حزمة `Firqah-Najiyah-1.0.0.apk` مرفَقة على [GitHub Release](https://github.com/SalehGNUTUX/Firqah-Najiyah/releases/tag/v1.0.0) (توقيع debug، لا يوجد توقيع نهائي بعد). زرّ تنزيل جديد في الصفحة الرئيسية للموقع (commit ea8cf72) يربط بـ`releases/latest`.
- **لم يُنجَز بعد:** أيقونة adaptive نهائية مصقولة (الحالية بدائية: لوغو مصغّر + خلفية خضراء صلبة)، splash screen مخصّص، توقيع الإصدار (keystore.properties)، CI/Fastlane/F-Droid.
